"""Test populator for coaching round 2 - touches ONE fork only.

Safety wrapper around ``populate.py`` for trying the coaching round 2
checklist against a single fork before unleashing it on every fork.
...
Usage (from this folder):
    ...
    python populate_coaching2_test.py --dry-run          # preview only
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Populate coaching round 2 issues on a SINGLE fork (test run)."
    )
    ...
    parser.add_argument("--data-file", default=COACHING2_DATA_FILE,
    ...
    print(populate.header(f"TEST RUN - coaching round 2 - target fork user: {args.fork_user!r}"))
