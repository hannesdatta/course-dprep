...
subtitle: "Data Preparation & Programming Skills (328059-M-6)"
author: "Hannes Datta"
date: 2026-03-09
format:
  pdf:
...
*updated 9 March 2026, 21:26 with revised order of the answer keys for MC questions*

## Overview

The exam for this course is a **120-minute on-campus computer exam** (course code 328059-M-6), taken in [TestVision](https://TilburgU.testvision.nl/online/kandidaten). It is a __closed-book exam without internet access__, though students may download selected cheat sheets from the exam’s introduction page. 
...
Please download the [`netflix_data`](https://github.com/hannesdatta/course-dprep/raw/main/material/exam/example_engineering/netflix_data.zip) file, containing an `.RData` environment that itself contains R objects (datasets). To receive points on this question, please create and upload one R file (.R) that performs the task as requested.

Important:

- Load the data using: load("name_of_the_file.RData") # use the correct file name as per your download
- Apply the Setup – Input – Transform – Output (S-I-T-O) principles used in class.
- Submit your work as a clean .R script (NOT RMarkdown). Ensure the code runs top to bottom.
- Do not include your name or student number anywhere in the code.
...
a) Some shows belong to multiple genres which are stored in the `genre` column as comma-separated string. Use regular expressions to create a new dummy column `is_action` that is `1` if the `genre` column contains "Action" genre and 0 otherwise.
...
b) Let's say we want to analyze how viewership for a specific show (SHOW_10) varies across different countries over time. Subset the viewership data for this show and convert the data to wide format.
...
a) For one country, estimate a regression of viewership_count on a dummy variable, set to 1 if the show is listed in the "Action" genre.

b) Then, adapt the code to run this regression for all countries using a loop, saving the results (country name, coefficient for dummy variable, p-value) in a table.

c) Save the combined results from (b) as regression_results.csv.
...
write_csv(effect_sizes, 'regression_results.csv')
```

*Note: you can also expect similar questions where you would have to plot at scale, or use wide --> long, long --> wide and imputation transformations (for details, see example questions in engineering tutorial).*
...
A)  One row per customer per week with aggregated ad exposure, total purchases, and churn status (flagged if no purchases in 90 days)
B)  One row per ad impression with variables for user ID, timestamp, and product purchased
C)  One row per customer per day with variables for total impressions, purchases, and weather in that region
...
C)  Duplicate all sources: one version in long format and one in wide format, maintained separately
D)  Prepare a core dataset at the correct unit of analysis, and reshape into long or wide format depending on the task

**Correct Answer: D**

**Explanation:**

-   **D is correct** because *Marketing Analytics: A Modern Toolkit* highlights that the **same derived dataset** can serve multiple purposes if it is first harmonized at the right unit of analysis. Analysts can then pivot between **long format** (suitable for regressions) and **wide format** (suitable for dashboards).
...
-   C is inefficient and risks divergence across duplicated datasets, which the book warns against.
...
C) Disagree, because small projects should avoid all infrastructure beyond Positron
D) Disagree, because the book stresses that tools add complexity and should be matched to project scope and needs

**Correct Answer: D**

**Explanation:** D is correct because *Marketing Analytics: A Modern Toolkit* emphasizes that “more is not always better.” Tools like Docker and CI/CD add learning costs and maintenance overhead. They are valuable for large, long-term projects but unnecessary for small, quick tasks. A is wrong because advanced tools may reduce efficiency in small projects. B is wrong because best practices must be adapted, not applied universally. C is wrong because small projects still benefit from basics like version control and folder structure.

#### Q2.2 (based on chapter 2) (3P)
...
B) The second student is right, because the book stresses reproducibility depends on locking package versions
C) Both students are equally right, since package management is optional
D) Neither is right, since reproducibility is not affected by package management

**Correct Answer: B**

**Explanation:** B is correct because *Marketing Analytics: A Modern Toolkit* explains that reproducibility requires code to run the same across time and machines. Tools like renv lock package versions, avoiding “it worked then, but not now” issues. A is wrong because constant updates can break old code. C is wrong because package management is not optional for reproducibility. D is wrong because package versions directly affect whether results can be replicated.

#### Q2.3 (based on chapter 2) (3P)
...
A) Keeping staged folders is better, because it preserves reproducibility and traceability of data transformations
B) Overwriting is best, since it minimizes clutter and storage costs
...
A) It is problematic, because scattering communication across multiple channels reduces transparency and reproducibility
B) It is reasonable, since lightweight tools like email are more efficient than formal boards
C) It is acceptable, provided that team members document progress individually in shared folders
D) It is effective, because agile teams rely more on conversation than on written artifacts

**Correct Answer: A**

**Explanation:** A is correct because the book emphasizes **centralizing communication in issues and boards**, ensuring transparency, traceability, and shared understanding. B is wrong because email creates fragmentation. C is wrong because scattered folder notes lack integration. D misinterprets Agile — while conversation matters, artifacts like boards are essential for documentation and accountability.

#### Q2.7 (based on chapter 5) (3P)
...
C) It is required, since Git automatically adds details from diffs anyway
D) It is problematic, because vague commit messages harm the usefulness of Git’s history for collaboration and reproducibility

**Correct Answer: D**

**Explanation:** D is correct because the book highlights that **clear, descriptive commit messages** are essential for making the project history understandable for teammates and your future self. A is wrong because code diffs alone are not always clear. B is wrong because even small commits should carry meaning. C is wrong because Git does not replace commit messages with diffs automatically.
...
> 
> Can you create a plot with `ggplot2` to shed some light on how the start of the global pandemic (let’s assume that was March 2020) affected music consumption (streams)?
> 
> Please render your RMarkdown as a PDF document. Please keep any code that you’re writing (e.g., to load the data, or to generate the plot -- including some minor data preparations) visible so I can learn from it (and grade it!)...!
...
### Exemplary question based on book (6P on the exam)
...
C) Clever code cannot be executed outside Positron
...
A) By always keeping a raw, untouched dataset and performing cleaning steps in code
B) By saving results only in final plots and tables
...
- Please use the Windows Command Prompt or Anaconda Prompt to run R or make from the command prompt. When using the command prompt through the terminal in Positron, it may happen that some of the installed packages cannot be found.
...
B) Neutral, since exploration is optional when datasets come from reliable company sources
C) Weak, because exploration provides necessary audits (e.g., missing values, outliers) to ensure the data makes sense
...
A) Team B, because generated files can always be recreated, making pipelines leaner and more scalable
B) Team A, because generated files are the most valuable artifacts
C) Team A, because reproducibility depends on storing every output
...
B) Risky, since deleting generated files makes reproducibility impossible
C) Efficient, since it prevents the need for automation
D) Wasteful, since generated files can be recreated from raw data and source code, making permanent storage unnecessary
...
Students will have access to **cheatsheets** during the exam, consisting of official cheatsheets used throughout the course (e.g., on Base R, on ``ggplot`), and one collective cheatsheet created by students.

### Official cheatsheets used throughout the course

The official cheatsheets used throughout the course cover the main tools -- **R**, **make**, and **Git**. Cheatsheets from previous years can already be downloaded [here](https://github.com/hannesdatta/course-dprep/raw/main/material/exam/cheatsheets/cheatsheets-exam.zip). Ahead of the exam, there will be a short Q&A session where you can suggest additional cheat sheet ideas to the instructor; the final set of official cheatsheets will then be compiled and published.

### Collective cheatsheet

Students are advised to prepare one **collective cheatsheet** in Google Docs. Once complete, the cheatsheet will be reviewed by the instructor, converted into PDF format, and shared with everyone as part of the final set of materials for the exam.

- [Link to Google Docs](https://docs.google.com/document/d/1nLEkypEmP3dTkljidmAiGmHptm0Bfe44LnwaYUnkdnU/edit?usp=sharing) (please use your Tilburg University email address to log in)
- Page limit: 50 pages
- Deadline to make your changes: 25 March 2026, 10am

::: callout
The instructor will review the document, make changes where required, and add it to the Cheatsheets.
:::
...
* **Get familiar with the exam environment**: practice using Windows, opening Positron, navigating with the command prompt, and setting working directories.
...
* **Save time on exam day**: before the exam officially starts, open the required software (e.g., Positron, Jupyter Notebook) and set working directories so you can begin immediately.
