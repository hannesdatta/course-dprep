...
-   Run `git checkout <SHA> -- my_answers.txt` (where SHA is the hash from git log) and inspect your directory. Is the `my_answers.txt` there again?

# Exercise 5: GitIgnore

-   Create a new file called `my_secret_key.txt`
-   `cd` into version-control-exercises.
-   Type `touch .gitignore` for macOs
-   Type `echo. > .gitignore` for PC
