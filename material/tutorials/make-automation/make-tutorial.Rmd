...
author: ""
date: "Last updated: 18 September 2025"
output:  
...
Imagine you're following a recipe to bake a cake. You don't want to mix the batter every time you need a slice—you want to bake it once and enjoy it whenever you like. **Automation with `make` works the same way!** It lets you define a "recipe" for your project so the computer can rebuild your results automatically whenever something changes.

In this tutorial, you'll learn how to:

- Organize your project files and directories.
- Set up a `makefile` to run your entire project with **one command**.

> **Why this matters**: No more manually rerunning code! `make` saves you time and ensures your results are always up to date.
...
**Exercise 1**  
As as starting point for this tutorial, we use [source code](https://github.com/hannesdatta/course-dprep/raw/main/material/tutorials/make-automation/run_antwerp.R) of an empirical project on the Airbnb housing market in Antwerp.

1. Download the source code as `run_Antwerp.R`, open the file in Positron, and run it. What output files does it return?
...
The lesson learned here is that we cannot only run R scripts from within Positron but also from the terminal. We'll use this knowledge when creating our first make file. 
...
The figure below shows how the `run_Antwerp` script is split into five modules, each with a specific role:

- **`download.R`**: Downloads raw listings and review data from [Inside Airbnb](http://insideairbnb.com).
- **`clean.R`**: Merges, preprocesses, and aggregates the data into `aggregated_df.csv`.
- **`pivot_table.R`**: Generates a pivot table with city neighborhoods as columns.
- **`plot_all.R`**: Aggregates reviews across all neighborhoods and creates a plot.
- **`plot_Antwerp.R`**: Creates a line chart for three neighborhoods in Antwerp.

![](./images/dependencies_Antwerp.png){width=50%}

> **Why this matters**: Splitting your code into modules makes it easier to update and debug. `make` ensures only the necessary steps are rerun when changes are made!


**Exercise 3: Splitting and Organizing Your Code**

Let's break this into smaller steps to make it easier!

#### Step 1: Split the R script into modules
- Open `run_Antwerp.R` in Positron.
- Copy the code under `### DOWNLOAD DATA` into a new file called `download.R`.
- Save `download.R` in the `src/data-preparation/` folder.

Repeat this process for the other sections:
- `### CLEAN DATA` → `clean.R` (save in `src/data-preparation/`)
- `### PIVOT TABLE` → `pivot_table.R` (save in `src/analysis/`)
- `### PLOT ALL` → `plot_all.R` (save in `src/analysis/`)
- `### PLOT ANTWERP` → `plot_Antwerp.R` (save in `src/analysis/`)

> **Tip**: Make sure each new file includes the necessary library imports at the top!

#### Step 2: Check if the scripts run
- Open each script in Positron and run it to ensure there are no errors.

#### Step 3: Define dependencies in the makefile
Update your makefile to include the following rules:
...
#### Step 4: Test your makefile
- Delete the files generated in the last iteration (`reviews.csv`, `listings.csv`, `aggregated_df.csv`, `pivot_table.csv`, `plot_Antwerp.pdf`, `plot_all.pdf`).
- Run `make -n` to preview the commands.
- Run `make` to execute the commands.

> **Check**: Does it work as expected? If not, double-check your file paths and dependencies.
