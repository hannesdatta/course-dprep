...
## DO: Creating a new folder for your project

- In the *Explorer pane*, click *Add a Folder*.
...
::: incremental
- files/directories (left)
- writing/editing source code (middle top) + output (middle bottom) 
- variables & plots (right)
- left menu bar: version control, extensions, ...
:::
...
* Running code (ctrl/cmd + ENTER, buttons, top-down)
...
#| code-overflow: wrap
filename = "https://raw.githubusercontent.com/hannesdatta/course-dprep/refs/heads/main/material/tutorials/r-bootcamp-rev/streams.csv"
download.file(filename, "video-view.csv")
```

```{r echo=TRUE, eval=TRUE}
library(tidyverse)
videos <- read_csv('video-view.csv', skip=1)
```
