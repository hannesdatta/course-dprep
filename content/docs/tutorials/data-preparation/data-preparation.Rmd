...
* Handle and preprocess string data (remove characters, cast to lower case, )
* Apply a variety of feature engineering techniques (grouping, rank, splitting, counter)
...
**Exercise 1**  
Merge all three files into a single data frame `df`. Keep in mind the following instructions:
...
**Exercise 5**  
Add a `ranking` column to `df` using the `dplyr` package. Although it is unlikely that some tracks have exactly the same number of streams, think about how you will deal with these edge cases (tip: run `?rank` to look up the documentation). Does your ranking always run from 1 to 200? Why is that? Investigate the root cause and fix it! 
...
**Exercise 6**   
As a last step, we call the `dcast()` function of the `reshape2` package to transform the data frame into the desired format (see the documentation for details!). Following these steps, it's your turn to create a function `reshape_songs()` that takes a data frame and a `country` as input and lists the genres (as columns) for all song ids (key; first column) in the respective country according to the specified format above. Note that the genres column may contain duplicates, that is: the same genre appears twice for the same song!
...
Last but not least, it's time to exploit our data wrangling efforts and seek interesting patterns in the data! Even though this is meant as more of an open-ended question, here are a couple of questions for your inspiration and get your started (feel free to come up other ones yourself!): 
...
Write up your analysis in R Markdown, illustrate your analysis with plots, and comment on the practical implications as well as the limitations of your findings. 
